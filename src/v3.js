...
Chart.prototype = {
    ...
    y:function(){
        ...
        if(this.options.type=="Column")
            to = this.data.category.length;
        if(this.options.axisInverted == true)
            return d3.scale.linear().domain([0,to]).range([this.height,0]);    
        else
    ...
    drawPoints:function(){
        ...
        for(var i=0;i<data.series.length;i++){
             this.context.chart.selectAll('point'+i).data(data.series[i]).enter().append("svg:circle").attr("class", function(d, i) {
              ...
              return d3.select(this).transition().attr('r', 4);
...
ColumnChart.prototype = {
    ...
    drawXAxis:function(){
        ...
        if(this.context.options.axisInverted==true){
            ticks.attr("transform",originFunc); 
            ticks.transition().duration(500).ease("basis").attr('transform',transFunc);   
        }else{
            ticks.attr("transform",transFunc); 
        }
    },
    ...
    drawPlots:function(){
        var data = this.context.data;
        var w    = this.context.width;
        var threshold = this.context.options.threshold;
        var self = this;
        
        var sc = data.series.length;
        var dc = data.series[0].length;
        var margin = 8;
        var gw = w/dc;
        ...
        for(var i=0;i<data.series.length;i++){
            ...
            if(self.context.options.axisInverted==true){
                ...
                for(var j = 0;j<data.series[i].length;j++){
                    var d = [0,data.series[i][j],data.series[i][j],0];
                    var rect = this.context.chart.selectAll('column'+i+'-'+j).data([d]).enter().append("svg:path");
                    rect.attr("d", d3.svg.line().interpolate("linear").x(function(d, idx) {
                            if(idx==0){
                                return self.context.x()(j)+sw*i+margin*(i+1);
                            }else if(idx==1){
                                return self.context.x()(j)+sw*i+margin*(i+1);
                            }else if(idx==2){
                                return self.context.x()(j)+sw*(i+1)+margin*(i+1);
                            }else if(idx==3){
                                return self.context.x()(j)+sw*(i+1)+margin*(i+1);
                            }
                    }).y(function(d,idx){
                            if(idx==0){
                                return self.context.y()(threshold);
                            }else if(idx==1){
                                return self.context.y()(d.val);
                            }else if(idx==2){
                                return self.context.y()(d.val);
                    ...
                    })).attr("class","column").attr("opacity","0").transition().duration(1000).attr("opacity","1");
                }
            }
        }
    },
    drawPoints:function(){
        ...
        for(var i=0;i<data.series.length;i++){
            ...
            if(self.context.options.axisInverted==true){
                ...
                for(var j = 0;j<data.series[i].length;j++){
                    this.context.chart.selectAll('point'+i+'-'+j).data(data.series[i]).enter().append("svg:circle").attr("cursor","pointer").attr("r", function(d, idx) {
                        return 3;
                    }).attr("cx", function(d, idx) {
                        if(self.context.options.axisInverted==true)
                            return self.context.x()(d.val);
                        else{
                            return self.context.x()(idx)+margin*(i+1)+(sw*i+sw/2);
                        }
                    ...
                    }).attr("opacity","0").attr("class","point").transition().duration(1000).attr("opacity","1");
